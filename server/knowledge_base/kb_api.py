...
def create_kb(knowledge_base_name: str = Body(..., examples=["samples"]),
              ...
              embed_model: str = Body("zhipu-api"),
              ) -> BaseResponse:
    ...
    try:
        ...
        msg = f"创建知识库出错： {e}"
        if "loop of ufunc does not support argument 0 of type NoneType" in str(e):
            msg = "创建知识库出错：本地嵌入模型不可用。请改用在线嵌入模型 zhipu-api，或先下载本地 embedding 模型。"
