...
def _direct_zhipu_reply_with_context(user_query: str, context: str, temperature: float = 0.7) -> str:
    """Direct Zhipu fallback when local LLM gateway returns empty output."""
    try:
        import os
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "你是一个严谨的知识库问答助手，请使用中文回答。"},
...
async def knowledge_base_chat(query: str = Body(..., description="用户输入", examples=["你好"]),
    ...
    if name_reply:
        async def name_reply_iterator() -> AsyncIterable[str]:
            yield json.dumps({"answer": name_reply, "docs": []}, ensure_ascii=False)
    ...
    async def knowledge_base_chat_iterator(
        ...
        if stream:
            ...
            if not answer.strip():
                # When the local gateway returns empty output, fallback to direct provider API.
                answer = _direct_zhipu_reply_with_context(query, context, temperature=temperature)
