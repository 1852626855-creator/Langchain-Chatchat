...
async def agent_chat(query: str = Body(..., description="用户输入", examples=["恼羞成怒"]),
    ...
    if name_reply:
        ...
        async def name_reply_iterator():
            yield json.dumps({"answer": name_reply, "final_answer": name_reply}, ensure_ascii=False)

        return EventSourceResponse(name_reply_iterator())

    def _local_tool_fallback(user_query: str) -> Optional[str]:
        """Fallback for basic time/date tool intents when LLM tool routing is unstable."""
        now = datetime.now()
        text = user_query.strip()
        if any(k in text for k in ["几点", "时间", "现在几时", "当前时间"]):
            return f"当前时间是 {now.strftime('%Y-%m-%d %H:%M:%S')}。"
        if any(k in text for k in ["几号", "日期", "哪一天", "今天是几月几号"]):
            return f"今天日期是 {now.strftime('%Y-%m-%d')}。"
        if any(k in text for k in ["星期", "周几", "礼拜几"]):
            week_map = ["一", "二", "三", "四", "五", "六", "日"]
            return f"今天是星期{week_map[now.weekday()]}。"
        return None

    fallback_answer = _local_tool_fallback(query)
    if fallback_answer:
        if user_id:
            add_chat_history(user_id, "user", query)
            add_chat_history(user_id, "assistant", fallback_answer)

        async def fallback_iterator():
            yield json.dumps({"answer": fallback_answer, "final_answer": fallback_answer}, ensure_ascii=False)

        return EventSourceResponse(fallback_iterator())
    ...
    async def agent_chat_iterator(
        ...
        # zhipu/chatglm models work better with JSON-action prompt format.
        if prompt_name == "default" and ("chatglm3" in model_name or "zhipu-api" in model_name):
            effective_prompt_name = "ChatGLM3"

        prompt_template = get_prompt_template("agent_chat", effective_prompt_name)
        ...
        if stream:
            async for chunk in callback.aiter():
                ...
                if data["status"] == Status.start or data["status"] == Status.complete:
                    ...
                    tools_use.append("\n```\n")
                    tools_use.append("工具名称: " + data.get("tool_name", "unknown"))
                    tools_use.append("工具状态: " + "调用失败")
                    tools_use.append("错误信息: " + data.get("error", "unknown error"))
            ...
            async for chunk in callback.aiter():
                ...
                if data["status"] == Status.error:
                    answer += "\n```\n"
                    answer += "工具名称: " + data.get("tool_name", "unknown") + "\n"
                    answer += "工具状态: " + "调用失败" + "\n"
                    answer += "错误信息: " + data.get("error", "unknown error") + "\n"
                    answer += "\n```\n"
                ...
                elif "llm_token" in data:
                    answer += data["llm_token"]

            response_text = final_answer or answer
            if not response_text.strip():
                response_text = "当前请求处理失败，请稍后重试。"
            if user_id and response_text:
                add_chat_history(user_id, "user", query)
                add_chat_history(user_id, "assistant", response_text)
